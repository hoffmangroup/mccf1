...
mccf1_calcu <- function(actualVector, predictedVector, fold=100) {
  ...
  for (i in 1:fold){
    ...
    sum_of_distance <- 0
    for (j in pos){
      temp_dist <- sqrt((mcc.nor_truncated[j] - 1)^2 + (f_truncated[j] - 1)^2)  
      sum_of_distance <- sum_of_distance + temp_dist
    }
  ...
  mccf1_metric <- 1 - (sums_no_na / length(sum_of_mean_distance)) / sqrt(2)
  ...
  bestThreshold <- thresholds[match(min(eu_distance, na.rm = T), eu_distance)]
  
  # output of the function is the MCC-F1 metric and the top threshold
  mccf1_result <- list(mccf1_metric = mccf1_metric, bestThreshold = bestThreshold)
  return(mccf1_result)
}
...
mccf1_plot <- function(actualVector, predictedVector, fold=100, .title="the MCC-F1 score curve",
  ...
  mccf1_df <- data.frame(F1 = f1, MCC.nor = mcc.nor) 
