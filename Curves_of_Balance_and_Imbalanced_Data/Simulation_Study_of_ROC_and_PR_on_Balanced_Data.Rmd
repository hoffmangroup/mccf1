...
# 1, 2 and 3 have same AUC
predicted_value_1 = c(rbeta(2000, 10, 1.5), rbeta(3000, 2, 4), rbeta(negative_num, 2, 4))
predicted_value_2 = c(rbeta(3000, 3.05, 3), rbeta(2000, 3, 4), rbeta(negative_num, 2, 4))
predicted_value_3 = c(rbeta(3000, 3.05, 3), rbeta(2000, 3, 4), rbeta(2000, 1, 12), rbeta(3000, 4, 4.5))
```
...
output <-cbind.data.frame(tpr, fpr, precision, recall, mcc.nor, f, auc, mcc_measure)
  return(output)
}
tpr_index = 1
fpr_index = 2
...
# ROC 
classification_plot(real, prediction, c(fpr_index, tpr_index), model_name, "ROC", "FPR", "TPR","5000:5000")
# PR
classification_plot(real, prediction, c(recall_index, precision_index), model_name,  "PR", "Recall", "Precision","5000:5000")
# mcc-f1
classification_plot(real, prediction, c(f_score_index, mcc.nor_index), model_name,  "mcc-f1", "F1", "MCC", "5000:5000")
# mcc-precision
...
```{r,echo=F, eval=T,cache=T, message=F,warning=F}
real = real_value
# need to substitute with predicted_value....
prediction = cbind(predicted_value_1, predicted_value_2, predicted_value_3)
model_name= c("type 1", "type 2", "type3")

# ROC 
classification_plot(real, prediction, c(fpr_index, tpr_index, auc_index), model_name, "ROC", "FPR", "TPR","5000:5000")
# PR
classification_plot(real, prediction, c(recall_index, precision_index), model_name,  "PR", "Recall", "Precision","5000:5000")
# mcc-f1
